...
const GalleryScreen: React.FC = () => {
  ...
  const fetchPhotos = async () => {
    try {
      ...
      // 2. Fetch Uploaded Photos (Safe Fetch)
      ...
      if (uploadsResult.error) {
        console.warn('Error fetching uploaded photos (Table might not exist yet):', uploadsResult.error);
      } else if (uploadsResult.data) {
    ...
    <div className="flex flex-col h-full bg-white pb-28">
      ...
      <header className="sticky top-0 z-30 flex items-center justify-between p-4 bg-white/90 backdrop-blur-md border-b border-slate-100">
        ...
        <div className="flex flex-col items-center flex-1 mx-2">
          ...
          <p className="text-xs text-text-muted font-medium">{photos.length} Photos</p>
        </div>
        <button className="flex size-10 items-center justify-center rounded-full hover:bg-slate-100">
      ...
      <section className="flex gap-3 px-4 py-4 w-full">
        <div className="flex flex-1 flex-col gap-1 rounded-xl bg-white p-3 border border-slate-200 items-center text-center shadow-sm">
          ...
          <p className="text-text-muted text-[10px] font-medium">Event Covers</p>
        </div>
      </section>
      ...
      <div className="flex-1 px-4 pb-12 masonry-grid mt-4">
        {loading ? (
          <div className="text-center text-gray-400 w-full col-span-2">Loading...</div>
        ) : photos.length === 0 ? (
          <div className="text-center text-gray-400 w-full col-span-2">No photos yet. Create an event to see it here!</div>
        ) : (
          photos.map(photo => (
            <div key={photo.id} className="masonry-item relative group overflow-hidden rounded-xl bg-slate-100 mb-4">
              ...
              <div className="absolute bottom-3 left-3 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity w-full pr-4">
      ...
      {/* Floating Upload - Kept for visual consistency, though functionality not requested yet */}
